...
class MyMplCanvas(FigureCanvas):
	...
	# We want the axes cleared every time plot() is called
        ...
        self.axes.set_title("sdkjghd")

        self.axes.set_xlabel( "My Label" ) 
    ...
    def update_figure(self, rangex, values):
        self.axes.plot(self.b[(0):(rangex)], values, '-')
        self.draw()

    def update_figure_lim(self, rangex, values, limd, limu):
        self.axes.plot(self.b[(0):(rangex)], values, '-')
        self.axes.set_autoscaley_on(False) 
        self.axes.set_ylim(limd, limu)
        self.draw()
